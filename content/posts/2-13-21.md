+++ 
draft = true
date = 2021-02-13T15:41:34-06:00
title = "Rate Limiting and Exponential Backoff in controller-runtime"
slug = "controller-runtime-rate-limiting" 
tags = []
categories = []
+++

# Rate Limiting and Exponential Backoff in controller-runtime
...
consumers to setup multiple controllers that are all handled under controller
...
4. Add the work to a queue and respond to the request to let it know that it
   will let the client know when it completes the work.
...
> prefers to make decisions at the last moment. I may prefer one communication
...
`controller-runtime` accepts this reconciliation loop via controller [builder
pattern]() implementation that allows a consumer to specify resources that
should trigger this reconciliation to run. Here is an example of a controller
that would be triggered by an CRUD operations on a `Pod`:
...
below (you also look directly at the
...
|   true  |      any     |  nil  |       Requeue with rate limiting.       |
|  false  |      >0      |  nil  | Requeue after specified `RequeueAfter`. |
...
not being a certain condition. `controller-runtime` has to accommodate for
...
The two rate limiters that are passed to it are
...
and
[`BucketRateLimiter`](https://github.com/kubernetes/client-go/blob/20732a1bc198ab57de644af498fa75e73fa44c08/util/workqueue/default_rate_limiters.go#L48).
The comments give us the helpful hint that the former is "per-item" while the
...
named `one` in the `default` namespace, and our enqueue delay grows
...
with an enqueue backoff that looks like this: 
...
only going to enqueue ~6 times.

![single-item-req](https://i.imgur.com/IZcbqph.png)
...
case maximum enqueues). If we only used the per-item rate limiter, that number
...
moment, there would be somewhere between 60,000 and 70,000 requeues _within the
first second_.

> Note: all of our measurements are approximate as we are not considering
> execution time of the actual reconciliation and related operations. However,
> the concepts still hold, most notably that the upper limit is unbounded.

![req-per-sec](https://i.imgur.com/wjEBeR0.png)

This is where the `BucketRateLimiter` come into play. It is a wrapper around the
...
`client-go`, the defaults passed to the `BucketRateLimiter` tell us that bucket
starts out with 100 tokens, which is also the maximum it can hold, and will be
refilled at a rate of 10 tokens per second. The 100 token limit is known as the
_maximum burst_ as it the cieling of requests that can happen at the same
...
![token-bucket](https://i.imgur.com/5V7ecFr.png)
...
enqueue immediately by the `BucketRateLimiter`, but the
`ItemExponentialFailureRateLimiter` is going to say "not so fast" because `0.005
* 2^0 = 0.005`, which is greater than `0`, so it will be selected by the
  `MaxOfRateLimiter` as the delay value to return.

However, in most cases, there will not be 10,000 unique objects all returning
failures at the same instant, and the enqueues per second will look like a curve
falling somewhere beneath the maximum controller enqueue limit established the
`BucketRateLimiter`.

## Using Your Own Rate Limiter

Now that we have explored the default implementation, it is time to consider
whether it is acceptable for your use case. If not, you can [supply your
own](https://github.com/kubernetes-sigs/controller-runtime/issues/631). The
controller builder we used when constructing our toy controller allows us to
...
- Overriding the default values passed to the rate limiter that
